...
#define GAZEBO_SIMULATION_RATE 1 // how many moves per second will that will be processed/simulated

// boolean expects sub_x and sub_y to determine if sub is at home position
#define SubIsHome(sub) (sub.sub_x == sub.sub_start_x && sub.sub_y == sub.sub_start_x)
...
int detect_survivors(assignment_3::Sensor &survivor_srv, int (&curr_world)[BOARD_H][BOARD_W], Sub&sub, Survivors&survivors);
...
void regenerate_moves(int (&current_world)[BOARD_H][BOARD_W], Sub&sub, std::vector<Sub>&subs);
...
int perform_move(Sub&sub, Survivors&survivors, int(&current_world)[BOARD_H][BOARD_W], std::vector<Sub> subs, ros::Rate rate);
...
int main(int argc, char *argv[])
{
	...
	sub.id=1;
	sub.sub_start_x=SUB_START_X;
	sub.sub_start_y=SUB_START_Y;
	...
	sub2.id=2;
	sub2.sub_start_x=SUB_START_X2;
	sub2.sub_start_y=SUB_START_Y2;
	...
	for (Sub& submarine : subs){
		int newSurvivorsDetected = detect_survivors(survivor_srv, current_world, submarine, survivors);
		if (newSurvivorsDetected)
		{
			...
			survivors.survivors_seen += newSurvivorsDetected;
		...
		regenerate_moves(current_world, submarine, subs);
	}
	...
	while (ros::ok())
	{
		...
		for (Sub& submarine : subs){
			perform_move(submarine, survivors, current_world, subs, rate);
		}
...
int perform_move(Sub&sub, Survivors&survivors, int(&current_world)[BOARD_H][BOARD_W], std::vector<Sub> subs, ros::Rate rate){
		...
		if (sub.q.empty())
		{
			...
			if ((survivors.survivors_saved) == SURVIVOR_COUNT)
			{
		...
		for(Sub&submarine:subs){
			if(submarine.sub_x==new_x&&submarine.sub_y==new_y&&submarine.id!=sub.id){
				//will collide, need to prevent collision
				regenerate_moves(current_world, sub, subs);
				ROS_INFO("Would result in collision new moveset needed");
				// retrieve next move and remove it from queue
				sub.next_move = std::string(sub.q.front());
				sub.q.pop();
				ROS_INFO("Next move is: %s", sub.next_move.c_str());
		...
		if (current_world[new_x][new_y] == SURVIVOR)
		{
			...
			survivors.survivors_saved++;
			std::cout << "Now have " << survivors.survivors_saved << " points of interest located" << std::endl;
		}
		...
		int newSurvivorsDetected = detect_survivors(survivor_srv, current_world, sub, survivors);
		if (newSurvivorsDetected)
		{
			// detected a survivor, change our planning to pick them up ASAP
			ROS_INFO("New point of interest(s) detected!");
		...
		ROS_INFO("Sub id: %s", std::to_string(sub.id).c_str());
		ROS_INFO("Sub x: %s", std::to_string(sub.sub_x).c_str());
		ROS_INFO("Sub y: %s", std::to_string(sub.sub_y).c_str());
		return -1;
	}
		...
		for (int i = 0; i < survivor_srv.request.sensorRange; i++)
			if (survivor_srv.response.eastRadar[i])
				if (curr_world[sub.sub_x][sub.sub_y + 1 + i] != SURVIVOR)
				{
					...
					ROS_INFO("Robot has detected a point of interest east!");
				}
	// west is col - 1
		...
		for (int i = 0; i < survivor_srv.request.sensorRange; i++)
			if (survivor_srv.response.westRadar[i])
				if (curr_world[sub.sub_x][sub.sub_y - 1 - i] != SURVIVOR)
				{
					newSurvivorsDetected++;
					curr_world[sub.sub_x][sub.sub_y-1-i] = SURVIVOR;
					ROS_INFO("Robot has detected a point of interest west!");
				}
	// north is row - 1
		...
		for (int i = 0; i < survivor_srv.request.sensorRange; i++)
			if (survivor_srv.response.northRadar[i])
				if (curr_world[sub.sub_x - 1 - i][sub.sub_y] != SURVIVOR)
				{
					...
					ROS_INFO("Robot has detected a point of interest north!");
				}
	// south is row + 1
		...
		for (int i = 0; i < survivor_srv.request.sensorRange; i++)
			if (survivor_srv.response.southRadar[i])
				if (curr_world[sub.sub_x + 1 + i][sub.sub_y] != SURVIVOR)
				{
						newSurvivorsDetected++;
						curr_world[sub.sub_x + 1 + i][sub.sub_y] = SURVIVOR;
						ROS_INFO("Robot has detected a point of interest south!");
				}
...
void generate_known_world(int (&world)[BOARD_H][BOARD_W], Sub&sub, std::vector<Sub>&subs)
{
	...
	file << "#define SUB_HOME_X " << sub.sub_start_x << ";\n";
	file << "#define SUB_HOME_Y " << sub.sub_start_y << ";\n";		
    ...
    for (std::size_t i = 0; i != subs.size(); i++){
		all_xpos.append(std::to_string(subs[i].sub_x));
		all_ypos.append(std::to_string(subs[i].sub_y));
		if (i != subs.size()-1) {
            all_xpos.append(", ");
			all_ypos.append(", ");
        }
	...
	std::stringstream ss;
    ss << "var xpos[" << subs.size() << "]:{0..Rows-1} = [" << all_xpos << "];\n";
    std::string xpos_result = ss.str();
	std::stringstream ss2;
	ss2 << "var ypos[" << subs.size() << "]:{0..Cols-1} = [" << all_ypos << "];\n";
    std::string ypos_result = ss2.str();
...
void regenerate_moves(int (&current_world)[BOARD_H][BOARD_W], Sub&sub, std::vector<Sub>&subs)
{
	...
	generate_known_world(current_world, sub, subs);
	...
	{
		ROS_INFO("Calculating a path to identify remaining points of interest");
